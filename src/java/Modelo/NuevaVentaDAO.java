...
//   101000001      2010001       301001    2024-03-28     14:51:09      70                  1
        ...
        String sqlInsFaltantes = "insert into faltantes (IdProducto, FechaApartir) values ";
//        String sqlUpdFaltantes = "update faltantes (IdProducto, FechaApartir) values ";
//        String sqlUpdFaltantesVal = null;
        String sqlInsFaltantesVal = null;
        boolean faltanteBandera = false;
        String sqlStock = "update detalle_ventas (IdDetalleVentas, IdVenta, IdProducto, Cantidad, Subtotal) values ";
        String linea = null;
            ...
            for (int i = 0; i < detallesVenta.size(); i++) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        stock = Float.parseFloat(stockS);
                        String stockMinS = rs.getString("stockMinimo");
                        stockMin = Float.parseFloat(stockMinS);
                    }
                ...
                */
                ...
                if(newStock <= stockMin){
                    String sqlSelFaltantes = "select * from faltantes where IdProducto  = " + IdProd;   
//                    String sqlInsFaltantes;

                    try {

                        ps = con.prepareStatement(sqlSelFaltantes);
                        rs = ps.executeQuery();
                        while (rs.next()) {
                            faltanteBandera = true;
                        }                   
                    } catch (SQLException | NumberFormatException e2) {
                        System.out.println("Error en la escritura faltantes");
                        System.out.println(e2.getMessage()); 
                    }        
                    
                    if(!faltanteBandera){                            
/*                        if(sqlUpdFaltantesVal == null){
                            sqlUpdFaltantesVal = "('" + IdProd + "','" + fechaVenta + "')";
                        }else{
                            sqlUpdFaltantesVal += ",('" + IdProd + "','" + fechaVenta + "')";                           
                        }
                    }else{*/
//                        sqlInsFaltantes = "insert into faltantes (IdProducto, FechaApartir) values ";
                        if(sqlInsFaltantesVal == null){
                            sqlInsFaltantesVal = "('" + IdProd + "','" + fechaVenta + "')";
                        }else{
                            sqlInsFaltantesVal += ",('" + IdProd + "','" + fechaVenta + "')";                           
                        }
                    }
            ...
            //if(sqlUpdFaltantesVal != null){
/*            if(!sqlUpdFaltantesVal.equals(null)){
                String sqlUpdFaltantesFin =sqlUpdFaltantes + sqlUpdFaltantesVal;
                ps = con.prepareStatement(sqlUpdFaltantesFin);
                ps.executeUpdate();
            }
        */
            if(sqlInsFaltantesVal != null){
                String sqlInsFaltantesFin =sqlInsFaltantes + sqlInsFaltantesVal;
                ps = con.prepareStatement(sqlInsFaltantesFin);
                ps.executeUpdate();
            }
